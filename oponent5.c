...
int main(int n_args, const char *ll_args[]){
  ...
  int ids_busties_opo = atoi(ll_args[13]);
  ...
  Bustia* bustia = (Bustia*) map_mem(ids_busties_opo);
...
while (joc->fi_usu==0 ) {  
  ...
  //chocat??
  pthread_mutex_lock(&bustia->mutex);
    if(bustia->chocat){
      waitS(sem_tauler);
      printf("missatge rebut desdel tron %d\n", index);
      signalS(sem_tauler);
    }
  pthread_mutex_unlock(&bustia->mutex);
